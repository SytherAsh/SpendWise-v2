...
- **Expected Deliverable:** Deployed Spring Boot service on the chosen platform
  (Render/Railway/Fly.io, finalized during setup per `docs/deployment.md`), all env vars
...
- **Grounded in:** `docs/deployment.md` "Deploying Spring Boot Backend", "Architecture Overview" (always-on preference), env var list.
...
- **Grounded in:** `docs/deployment.md` "Deploying FastAPI ML Service", `FASTAPI_ML_URL` note; `docs/architecture.md` "Internal access only" note.
...
- **Grounded in:** `docs/deployment.md` "Deploying Frontend" (Vercel auto-deploys, no manual step).
...
- **Estimated Complexity:** Medium
- **Depends on:** E0-S2-T6
- **Grounded in:** `docs/security.md` Supabase Row-Level Security; `docs/deployment.md` Supabase (free tier).
...
- **Grounded in:** `docs/deployment.md` Monitoring Setup (Sentry); env var lists (`SENTRY_DSN`).
...
- **Expected Deliverable:** Unauthenticated endpoint checking Supabase connectivity and
  FastAPI ML reachability, returning the exact shape from `docs/deployment.md`.
...
- **Grounded in:** `docs/deployment.md` "Health Check Endpoint" (exact response shape, unauthenticated, not in `docs/api.md`).
...
- **Grounded in:** `docs/deployment.md` Monitoring Setup (UptimeRobot) + Health Check Endpoint section.
...
- **Grounded in:** `docs/deployment.md` "Distributing Android APK".
...
- **Expected Deliverable:** A completed checklist (copy of `docs/security.md`'s API
...
- **Grounded in:** `docs/security.md` API Security Checklist (verbatim list).
...
- **Grounded in:** `docs/deployment.md` CD section ("Manual — developer reviews CI results and deploys manually").
