...
class TransactionServiceImplTest {
    ...
    void listFirstPageDoesNotResolveACursor() {
        given(transactionRepository.listPage(eq(userId), eq(null), eq(false), eq(null), eq(null), eq(null), eq(null), eq(51)))
                .willReturn(List.of(sampleTransaction()));

        TransactionPage page = service.list(userId, 50, null, null, false, null, null);
    ...
    void listResolvesCursorToADateBeforeSeeking() {
        ...
        given(transactionRepository.listPage(userId, null, false, null, null, cursorDate, cursor, 51)).willReturn(List.of());

        service.list(userId, 50, cursor, null, false, null, null);

        verify(transactionRepository).listPage(userId, null, false, null, null, cursorDate, cursor, 51);
    }
    ...
    void listSetsHasMoreAndNextCursorWhenMoreRowsThanLimitAreReturned() {
        ...
        given(transactionRepository.listPage(eq(userId), eq(null), eq(false), eq(null), eq(null), eq(null), eq(null), eq(2)))
                .willReturn(List.of(first, second));

        TransactionPage page = service.list(userId, 1, null, null, false, null, null);
    ...
    void listPassesUncategorizedOnlyThroughToRepository() {
        given(transactionRepository.listPage(eq(userId), eq(null), eq(true), eq(null), eq(null), eq(null), eq(null), eq(51)))
                .willReturn(List.of());

        service.list(userId, 50, null, null, true, null, null);

        verify(transactionRepository).listPage(userId, null, true, null, null, null, null, 51);
    }
