...
- A second `JdbcTemplate` bean, injected only via `@Qualifier("jobsJdbcTemplate")`, bypasses RLS entirely. Four categories of caller are sanctioned, each for the specific cross-user read/write it needs, never a blanket bypass (see `com.spendwise.common.db.JobsDataSourceConfig`'s own javadoc for the authoritative list):
  - Background job classes (`TransactionRepository.findAllUncategorized`, `MlCorrectionRepository.findAllCorrections`, and the equivalent bulk reads for Epic 5's alert evaluator, Epic 6's recurring-detection pass, and Epic 8's recommendation generator) — all exposed to their consuming module through the owning module's normal service interface, never a new cross-module dependency. **As of ADR-018 (2026-07-19, `docs/spec/decisions.md`)** these jobs are no longer `@Scheduled`-annotated — `com.spendwise.common.schedule.DynamicJobScheduler` invokes them on a schedule read from the `job_schedules` table — but they remain the same cross-user jobs this category always meant.
  - **Added in Epic 11:** `com.spendwise.admin.AdminRepository`'s request-scoped reads (and the erasure flow's writes). Unlike every case above, Admin's need isn't a scheduled job — it's that Admin has no per-request "current user" to scope RLS to by construction: `GET /admin/users` must enumerate every user, and `GET /admin/logs` reads the system-wide `admin_logs` table, which no module's RLS-scoped connection could return regardless. Admin's *other* reads (`GET /admin/users/:id`'s per-user transaction/budget/alert data) need no bypass at all — they call `TransactionService`/`BudgetService`/`AlertsService` with the target user's id, exactly like Budget or Alerts already do, and those services scope RLS to whatever id is passed in.
  - **Added ML strategy phase (2026-07-19):** `com.spendwise.common.db.AdminEventLog` — writes the same system-wide `admin_logs` table Admin already reads cross-user, on behalf of whichever background job calls it to record a run/failure event (ADR-016). Not a new independent caller pattern, just the write side of a table this role already has sanctioned read access to.
  - **Added ML strategy phase (2026-07-19):** `com.spendwise.common.schedule.JobScheduleRepository` (ADR-018) — reads/writes the new `job_schedules` table, another system-wide, non-user table, from a `TaskScheduler` thread with no RLS session of its own by construction, same reasoning as `AdminRepository` above.
