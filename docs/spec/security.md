...
- A second `JdbcTemplate` bean, injected only via `@Qualifier("jobsJdbcTemplate")`, bypasses RLS entirely. Two categories of caller are sanctioned, each for the specific cross-user read/write it needs, never a blanket bypass:
  - `@Scheduled` job classes (`TransactionRepository.findAllUncategorized`, `MlCorrectionRepository.findAllCorrections`, and the equivalent bulk reads added for Epic 5's alert evaluator, Epic 6's recurring-detection pass, and Epic 8's recommendation generator) — all exposed to their consuming module through the owning module's normal service interface, never a new cross-module dependency.
