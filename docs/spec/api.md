...
| Method | Path | Description | Auth |
| --- | --- | --- | --- |
| GET | `/transactions` | List transactions (paginated, 50/page, cursor-based); supports optional filters: `category` (category_id, or the literal `uncategorized`), `direction` (`credit` or `debit`), `from`, `to` (ISO 8601 dates), `sort` (`date_desc` default, or `amount_desc`), `search` (free text) | User |
...
| DELETE | `/transactions/:id` | Soft-delete a transaction (sets `deleted_at`; not a hard SQL DELETE) — it disappears from every list/detail/analytics/budget/alerts read immediately. `204` on success; `404` (existence-hiding, not `403`) if absent, already deleted, or owned by a different user | User |
...
>
> **Recategorize-on-rename follow-up (ADR-020, ML strategy phase, 2026-07-20):** the frontend calls `POST /categorization/recategorize` (see `/categorization` below) with the transaction's own identity right after each `PUT /transactions/:id/payee` succeeds, so the rename also updates `category_id` for every transaction sharing that identity — not just its display name. Best-effort from the caller's perspective, same posture as `POST /alerts/reevaluate` below.
>
...
>
> **`search` (added for the Transactions page search box):** free-text filter, case-insensitive substring match (`ILIKE %search%`) against `recipientName`, `upiId`, `note`, or the assigned category's name; omitted or blank applies no filter. Independent of and ANDs together with `category`/`direction`/`from`/`to` — a search still respects an active category tile or date range rather than searching the user's entire history unscoped. Not supported on the `sort=amount_desc` bounded read.
>
> **Soft delete (added V17, Transactions page redesign):** every `GET /transactions` and `GET /transactions/:id` read excludes soft-deleted rows (`deleted_at IS NOT NULL`) — see `DELETE /transactions/:id` above. This also applies to every other module's read of `transactions` (budget progress, alerts/recurring-payment detection, analytics, ML correction retraining), so a deleted transaction disappears everywhere consistently, not just from this list.
...
> **Served by the Transaction module** — same module that already owns `recipient_canonical`/`recipient_canonicalization_overrides` (ADR-013/014), not a new module (ADR-015). Candidates are populated by `RecipientCanonicalizationSweep` from the same `/normalize-recipients` ML call it already makes on its weekly run (or an admin-triggered manual run) — `ambiguous_groups` is a new field on that response, not a second FastAPI call. A candidate already resolved against one anchor is never re-suggested against a different one. After a successful `resolve`, call `POST /alerts/reevaluate` (see `/alerts` below) to reflect the corrected identity in recurring-payment detection and budget alerts immediately rather than waiting for the next scheduled sweep, and (ADR-020) `POST /categorization/recategorize` (see `/categorization` below) once per confirmed `same: true` candidate identity, so the merge also updates those transactions' categories.
>
> Each candidate carries a `reason` explaining why the clustering flagged it: `fuzzy_near_miss` (near-miss similarity just below the auto-merge threshold) and `prefix_ambiguous` (a bare name that could belong to two or more fuller names) from ADR-015, plus `upi_handle_variant` (a name-derived UPI handle such as `AASHAYJ2` matching a spelled name's first token) and `short_prefix` (a 3–5 char truncation below the auto-merge prefix floor, e.g. `ALP` → `ALPHA VIJAY RANE`) added by ADR-019's Phase A enriched detectors. `reason` is a free-form string — clients display it but must not switch on the exact set, which grows over time.

### `/categorization` — Categorization (ADR-020, ML strategy phase, 2026-07-20)

| Method | Path | Description | Auth |
| --- | --- | --- | --- |
| POST | `/categorization/recategorize` | Body `{"recipient_name": "...", "upi_id": "..."}` (both nullable — an identity only needs one). Re-runs ML categorization for every transaction owned by the caller sharing that (recipient_name, upi_id) identity, using its current `recipient_canonical` as the prediction input. Never overwrites a transaction whose category was manually corrected (`assigned_by = 'user'`) — same guard `PUT /transactions/:id/category` relies on. Returns `{"recategorized": n}` — transactions re-attempted, not necessarily changed. | User |

> **Why this endpoint exists:** renaming/merging a payee (`PUT /transactions/:id/payee`, or `POST /payee-merge-queue/resolve`) only ever updated `recipient_canonical` — a display/grouping field — with no effect on `category_id`. Called by the frontend as a second, independent, best-effort request right after either of those succeeds, same pattern as `POST /alerts/reevaluate` above (see ADR-020 in `docs/spec/decisions.md` for why this is frontend-orchestrated rather than a backend call from the Transaction module — it would create a circular module dependency, since Categorization already depends on Transaction).
