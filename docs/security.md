...
- The default `DataSource`/`JdbcTemplate` — used by every controller, service, and repository in the app via unqualified injection — is completely unaffected and stays fully RLS-enforced, exactly as before.
...
> **Incident (first real CI run, 2026-07-02):** landing this without also tuning the jobs pool broke every *other* integration test's Spring context, not just ones touching a scheduled job. Cause: `CategorizationRetryJob`'s `@Scheduled(fixedRate = ...)` had no `initialDelay`, so it fired the instant each test's app context started — and in every context except the one that explicitly bootstraps `spendwise_jobs`, that first query has nothing to connect to. Hikari's `minimumIdle` defaults to `maximumPoolSize` (10), so its background connection-adder thread kept retrying, forever, trying to keep the pool warm against a role that doesn't exist — starving the whole JVM of CPU/threads on the CI runner and degrading completely unrelated endpoints (e.g. `/auth/otp/verify`) to ~30s per request (Hikari's default `connectionTimeout`). Fixed with an `initialDelay` on the job (also just correct behavior — a full system-wide sweep on every app restart was never intended) and `minimumIdle(0)` + a small `maximumPoolSize` on the jobs pool, so a missing-role environment fails once on actual demand instead of retrying in the background indefinitely. **Lesson for any future second connection pool:** a pool with `minimumIdle > 0` against credentials that might not exist in every environment is a resource-exhaustion risk, not just a "one query fails" risk.
